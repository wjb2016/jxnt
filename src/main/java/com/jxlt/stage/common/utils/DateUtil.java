...
public final class DateUtil {
	 ...
	 * @return
	 */
	public static String sortFormat(Date date){
	 ...
	 * @return
	 */
	public static Date sortParse(String date) throws ParseException{
	...
	public static int getBetweenDays(Date min, Date max){
		...
		//计算相差天数，忽略时分秒
		try {
			Date mx  = parse(sortFormat(max), "yyyy-MM-dd");
			Date mi = parse(sortFormat(min), "yyyy-MM-dd");
	 ...
	 * @return
	 */
    public static boolean isBirthMonth(Date birth){
		boolean result = false;
		if(birth == null)
			return result;
		try{
			String bM = sortFormat(birth).substring(5, 7);
			String nM  = sortFormat(new Date()).substring(5, 7);
			if(nM.equals(bM))
				result = true;
		}catch(Exception e){
			e.printStackTrace();
